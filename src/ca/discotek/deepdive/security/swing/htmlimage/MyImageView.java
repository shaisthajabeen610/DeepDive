...
public class MyImageView extends View {
     ...
     */
    public MyImageView(Element elem, WarNode warNode, HtmlFile htmlFile) {
        super(elem);
...
//        System.out.println(src);
    ...
    public synchronized boolean waitForAll(long ms) throws InterruptedException {
        ...
        while (!done) {
            Thread.sleep(ms);
            int flags = Toolkit.getDefaultToolkit().checkImage(image, -1, -1, new JLabel());
...
//    }
    ...
    private void loadImage() {
        String text = (String) getElement().getAttributes().getAttribute(HTML.Attribute.SRC);
        Image newImage = null;
...
//            
            ...
            String requestChunks[] = text.split("/");
            String imageChunks[];
            
            ImageFile images[] = warNode.war.getImageFiles();
            for (int i=0; i<images.length; i++) {
//                System.out.println(images[i].path + "  and  " + text);
                imageChunks = images[i].path.split("/");
                ...
                if (chunks.length == imageChunks.length) {
                    boolean found = true;
                    for (int j=0; j<chunks.length; j++) {
                        if (!chunks[j].equals(imageChunks[j])) {
                            found = false;
                            break;
                        }
                    }
                    if (found) {
                        newImage = Toolkit.getDefaultToolkit().createImage(images[i].bytes);
                        MediaTracker tracker = new MediaTracker(new JLabel());
                        tracker.addImage(newImage, 0);
                        
                        try { tracker.waitForAll(); }
                        catch (Exception e) {
                            System.out.println("Couldn't load image: " + text);
                        }
//                        System.out.println(images[i].path + "  and  " + text + ": " + images[i].bytes + ": " + newImage.getHeight(null) + ", " + newImage.getWidth(null));
//
//                        javax.swing.JFrame f = new javax.swing.JFrame();
...
//                        f.setVisible(true);
                        ...
                        if (newImage != null) {
                            ...
                            width = newImage.getWidth(label); 
                            height = newImage.getHeight(label);
                        }
                        
                        break;
                    }
                }
//                if (images[i].path.equals(text)) {
//                    newImage = Toolkit.getDefaultToolkit().createImage(images[i].bytes);
//                    break;
//                }
            }
