...
public class IntInstructionDescriptorEditor extends JPanel implements InstructionDescriptionEditor {

    OpcodeEditor opcodeField;
    EnabledButtonWrapper opcodeWrapper;
    
    JTextField operandField;
    EnabledButtonWrapper operandWrapper;

    public IntInstructionDescriptorEditor() {
        buildGui();
    }
    ...
    void buildGui() {
        ...
        opcodeField = new OpcodeEditor(OpcodeEditor.IntInstructionOpcodes);
        ...
        add(new JLabel("Operand"), labelGbc);
        operandField = new JTextField(3);
        operandWrapper = new EnabledButtonWrapper(operandField);
        operandField.setEnabled(operandWrapper.isEnabled());
        add(operandWrapper, fieldGbc);
    }
    
    public IntInstructionDescriptor getDescriptor() {
        IntInstructionDescriptor d = new IntInstructionDescriptor();
        
        if (opcodeWrapper.isEnabled())
            d.setOpcode(opcodeField.getOpcode());
        
        if (operandWrapper.isEnabled())
            d.setOperand(Integer.parseInt(operandField.getText()));
        
        return d;
    }
    
    public void setDescriptor(AbstractInstructionDescriptor d) {
        setDescriptor( (IntInstructionDescriptor) d);
    }
    
    public void setDescriptor(IntInstructionDescriptor d) {
        ...
        int operand = d.getOperand();
        if (operand > -1) {
            operandField.setText(Integer.toString(operand));
            operandWrapper.setEnabledButtonSelected(true);
        }
    }
